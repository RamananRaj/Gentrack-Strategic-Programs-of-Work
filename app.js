...
function renderNewsletter(){
  ...
  if(on('weekly')){ const numTd='padding:6px 8px;font-size:12px;color:#94a3b8;border-bottom:1px solid #eef2f7;vertical-align:top;width:24px;text-align:right;font-weight:700'; const itTd='padding:6px 10px;font-size:12.5px;color:#334155;border-bottom:1px solid #eef2f7;vertical-align:top';
    const blk=(t,a,flag)=>(on(flag)&&a&&a.length)?`${h3(t)}<table cellspacing="0" cellpadding="0" style="border-collapse:collapse;width:100%;margin:0 0 12px"><tbody>${a.map((x,n)=>`<tr><td style="${numTd}">${n+1}</td><td style="${itTd}">${esc(x)}</td></tr>`).join('')}</tbody></table>`:''; const whtml=blk('Accomplished This Week',DATA.weekly.accomplishments,'weekly.accomplished')+blk('Planned Next Week',DATA.weekly.planned,'weekly.planned')+blk('Blockers / Issues',DATA.weekly.blockers,'weekly.blockers'); if(whtml) B.push(whtml); }
  ...
  if(on('risks')){ const th='padding:7px 10px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.4px;color:#64748b;border-bottom:2px solid #e2e8f0;font-weight:700'; const td='padding:7px 10px;font-size:12.5px;color:#334155;border-bottom:1px solid #eef2f7;vertical-align:top';
    ...
    B.push(h3('Risks & Watch Items')+`<table cellspacing="0" cellpadding="0" style="border-collapse:collapse;width:100%"><thead><tr><th style="${th};white-space:nowrap">RAG</th><th style="${th}">Risk</th><th style="${th}">Impact</th><th style="${th}">Mitigation</th></tr></thead><tbody>${rows}</tbody></table>`); }
